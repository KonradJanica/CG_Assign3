...
Terrain::Terrain(const GLuint &program_id, const int &width, const int &height) : terrain_program_id_(program_id), width_(width), height_(height) {
  ...
  // Terrain
  // 1st buffer
  texture_ = LoadTexture("textures/water01.jpg");
...
void Terrain::GenerateTerrain(std::vector<glm::vec3> &vertices, std::vector<glm::vec3> &normals,
  ...
  if (!is_road) {
  	// @Mitchell - Note this alogirthm is inspired by the one found here
  	// http://www.lighthouse3d.com/opengl/terrain/index.php3?particle
  	// to create heightmaps by picking a point and increasing heights around that point
